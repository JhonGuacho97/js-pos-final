...
import { InputGroup, Modal, Button } from "react-bootstrap-v5";
...
const CreateSubProductModal = (props) => {
    ...
    const onVariationTypeChange = (obj) => {
        ...
        setFormInput((prev) => ({
            ...
            code: prev.code
                ? prev.code
                : `${commonData.product_code}-${upperCase(obj?.label || "")}`,
        }));
        setErrors({});
    };
    ...
    const handleValidation = () => {
        ...
        if (!formInput["variation_type"]) {
            ...
            validationErrors['code'] = getFormattedMessage('product.input.variation-code.validate.label');
