...
const UserForm = (props) => {
    ...
    && (singleUser[0].default_warehouse_id || '') === (userValue.default_warehouse_id || '')

    const [selectedRole] = useState(singleUser && singleUser[0] ? ([{
        label: singleUser[0].role_id.label[0], value: singleUser[0].role_id.value[0]
    }]) : null);

    const [selectedWarehouse] = useState(singleUser && singleUser[0] && singleUser[0].default_warehouse_id ? ([{
        label: singleUser[0].default_warehouse_name, value: singleUser[0].default_warehouse_id
    }]) : null);
            ...
            <div className='card-body'>
                <Form>
                    <div className='row'>
                        ...
                        <div className='col-md-6 mb-3'>
                            <ReactSelect title="Almacén por defecto"
                                         placeholder="Sin asignar (usa el almacén global)"
                                         defaultValue={selectedWarehouse}
                                         data={warehouses} onChange={onWarehouseChange}
                                         isRequired/>
                            <span className='text-muted d-block fs-small mt-1'>
