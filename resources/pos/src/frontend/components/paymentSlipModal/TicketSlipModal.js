    ...
    * { 
    ...
    const handleShareAsText = () => {
        ...
        const items = saleDetails?.sale_items?.map(item => {
            const variationLabel = item.product?.variation_type?.name;
            const itemName = variationLabel ? `${item.product.name} (${variationLabel})` : item.product.name;
            return `- ${itemName} x${item.quantity} = $${(item.quantity * calculateProductCost(item.product_price)).toFixed(2)}`;
        }).join('\n');
                    ...
                    <table style={{
                        ...
                        <tbody>
                            {saleDetails?.sale_items?.map((item) => {
                                const managePresentations = item.product?.manage_presentations ?? false;
                                ...
                                const totalPriceProductPresentation = item.product_presentation
                                    ? price * Number(item.presentation_quantity)
                                    : 0;

                                return (
                                    <tr key={item.id}>
                                        <td style={{ paddingTop: "2px" }}>
                                            {managePresentations && item.product_presentation?.variation_type?.name
                                                ? `${item.product.name} (${item.product_presentation.variation_type.name})`
                                                : item.product?.variation_type?.name
                                                    ? `${item.product.name} (${item.product.variation_type.name})`
                                                    : item.product.name}
                                        </td>
