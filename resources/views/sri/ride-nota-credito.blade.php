...
<body>
    ...
    <table class="header-table">
        <tr>
            ...
            <td style="width: 40%;">
                <div class="doc-box">
                    <div class="box-title">Nota de Crédito</div>
                    <div class="label-line">N° de comprobante</div>
                    <div class="value-line">{{ $comprobante->numeroComprobante() }}</div>

                    <div class="label-line">Número de autorización</div>
                    <div class="small">{{ $comprobante->numeroAutorizacionRide() }}</div>

                    <div class="label-line">Fecha y hora de autorización</div>
                    <div class="small">{{ $comprobante->fecha_autorizacion?->format('d/m/Y H:i:s') ?? 'Pendiente' }}</div>
                    ...
                    <div class="clave-acceso">{{ $comprobante->clave_acceso }}</div>
                </div>
            </td>
        </tr>
    </table>
        ...
        <tr>
            <td style="width: 50%; vertical-align: top; padding-right: 4px;">
                <div class="box">
                    ...
                    <table style="width:100%;">
                        <tr>
                            <td colspan="2">Razón Social / Nombres:
                                <strong>{{ $creditNote->customer->razonSocialSri() }}</strong></td>
                        </tr>
                        <tr>
                            <td style="width: 50%;">Identificación: <strong>{{ $creditNote->customer->identificacionSri() }}</strong></td>
                            <td>Fecha Emisión: {{ optional($creditNote->date)->format('d/m/Y') }}</td>
                        </tr>
                    </table>
                </div>
            </td>
            <td style="width: 50%; vertical-align: top; padding-left: 4px;">
                <div class="box">
                    ...
                    <table style="width:100%;">
                        <tr>
                            <td style="width: 50%;">Tipo: <strong>{{ $creditNote->tipo_comprobante_modificado }}</strong></td>
                            <td>N°: <strong>{{ $creditNote->numero_comprobante_modificado }}</strong></td>
                        </tr>
                    </table>
                </div>
            </td>
    ...
    <div class="motivo-box">
        <strong>Motivo:</strong> {{ $creditNote->motivo }}
    </div>
        ...
        <tbody>
            @foreach ($creditNote->creditNoteItems as $item)
                <tr>
                    ...
                    <td style="text-align:right;">{{ number_format($item->tax_value ?: $creditNote->tax_rate, 0) }}%</td>
            ...
            <td style="width: 45%; vertical-align: top;">
                <table class="totales-table" style="width:100%;">
                    <tr>
                        ...
                        <td class="value">${{ number_format($creditNote->subtotalSinIvaSri(), 2) }}</td>
                    </tr>
                    <tr>
                        <td class="label">Descuento</td>
                        <td class="value">${{ number_format($creditNote->discount ?? 0, 2) }}</td>
                    </tr>
                    @php
                        // Mismo criterio que el RIDE de Factura: la
                        // tarifa vive por producto, se suma el IVA real
                        // de cada línea, no un campo global.
                        $ivaTotal = $creditNote->creditNoteItems->sum(fn ($item) => $item->valorIvaSri());
                        $itemConIva = $creditNote->creditNoteItems->first(fn ($item) => ($item->tax_value ?: $creditNote->tax_rate) > 0);
                        $tarifaMostrada = $itemConIva ? ($itemConIva->tax_value ?: $creditNote->tax_rate) : ($creditNote->tax_rate ?? 0);
                    @endphp
                    <tr>
                    ...
                    <tr class="total-row">
                        <td class="label">Total Nota de Crédito</td>
                        <td class="value">${{ number_format($creditNote->grand_total ?? 0, 2) }}</td>
                    </tr>
                </table>
            </td>
        </tr>
