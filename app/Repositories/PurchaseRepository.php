...
class PurchaseRepository extends BaseRepository
{
    ...
    public function calculationPurchaseItems($purchaseItem)
    {
        ...
        // monetario (junto al costo propio de esa presentación). Al final de
        ...
        // siga funcionando exactamente igual que hoy, sin cambios. Mismo
        // patrón que ya usa SaleRepository::calculationSaleItems().
        $presentationQuantity = $purchaseItem['quantity'];
        $equivalence = 1;
        ...
        $purchaseItem['presentation_equivalence'] = $equivalence;
        ...
        // inventario (ej: 2 cajas * 24 = 48), para que el stock y todo lo
        // demás que consume 'quantity' funcione sin cambios.
        $purchaseItem['quantity'] = $presentationQuantity * $equivalence;
    ...
    public function storePurchaseItems($purchase, $input)
    {
        foreach ($input['purchase_items'] as $purchaseItem) {
            ...
            // Se suma el stock con la MISMA cantidad ya convertida a
            // unidades base que se acaba de guardar arriba (ej: 1 caja de
            // 24 -> quantity=24) -- antes esto se hacía en storePurchase()
            // releyendo el input crudo del formulario (ej: "1"), que nunca
            // pasaba por la conversión de presentación, así que el stock
            // quedaba sumado de menos y luego, al eliminar la compra, la
            // resta fallaba porque no había suficiente stock para descontar.
            manageStock($purchase->warehouse_id, $items['product_id'], $items['quantity']);
        }
        ...
        try {
            ...
            foreach ($input['purchase_items'] as $key => $purchaseItem) {
                ...
                $purchaseItemArr = Arr::only($purchaseItem, [
                    ...
                    'sub_total', 'product_presentation_id', 'presentation_quantity', 'presentation_equivalence',
                ]);
