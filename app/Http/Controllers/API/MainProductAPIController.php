...
class MainProductAPIController extends AppBaseController
{
    ...
    public function store(CreateMainProductRequest $request): MainProductResource
    {
        ...
        try {
            ...
            if ($input['product_type'] == 2) {
                ...
                if (!empty($input['manage_presentations']) && $input['manage_presentations'] != '0' && !empty($input['presentations'])) {
                    $presentations = json_decode($input['presentations'], true) ?: [];
                    foreach ($presentations as $sort => $presentation) {
                        \App\Models\ProductPresentation::create([
                            ...
                            'is_base_unit' => !empty($presentation['is_base_unit']),
                            'is_default' => !empty($presentation['is_default']),
                            'sort' => $sort,
                        ]);
                    }
                }
    ...
    public function update(UpdateMainProductRequest $request, $id): MainProductResource
    {
        ...
        foreach ($products as $product) {
            // El código de cada variante es propio y editable desde su
            // propio formulario -- ya no se pisa acá al editar el
            // producto principal (antes se recalculaba pegando
            // product_code + nombre de la variante en cada guardado).
